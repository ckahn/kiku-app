...
- **IndexedDB** (`kiku-offline`, via `idb`): stores `episodes`, `segments` (composite key `[episodeId, segmentIndex]`, index `by-episode`), `studyGuides`, `downloads` (plus `outbox` since M4). Zod-validated boundary in `store.ts`: writes `parse` (throw), reads `safeParse` (corrupt row = treated as missing). `deleteEpisodeData` cascades all five stores in one transaction, including the episode's queued outbox entries.
...
- **`outboxStore.ts`** — a `downloadStore`-style client singleton: enqueue coalesces per-target (last-write-wins), concurrent drains share one retained in-flight replay promise, a same-tab write lock (`withTargetWriteLock`) keeps a fresh online write and a replay of the same target from racing, and FIFO `replay()` runs on the `online` event (installed by `ensureOutboxInitialized`, also mounted app-wide via `PendingChangesIndicator` in `src/app/layout.tsx`).
...
- **`PendingChangesIndicator`** (root layout) — "N change(s) waiting to sync" plus a manual "Retry now" drain and a dismiss control for the sticky replay error; both controls meet the 44px/pointer rules. Both study-status controls keep a local optimistic mirror of the status so a queued offline change (no refresh) doesn't leave their labels or toggle direction stale.
