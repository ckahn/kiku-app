...
describe('tapSegment', () => {
  it('loop off — seeks to the tapped segment', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));

    act(() => { result.current.controls.tapSegment(SEG2.id); });

    expect(engineMock.seek).toHaveBeenCalledWith(4.9); // segmentStartSec(SEG2)
    expect(result.current.state.loopRange).toBeNull();
  });

  it('inside band — seeks without changing the range', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
    ...
    act(() => { result.current.controls.tapSegment(SEG2.id); }); // SEG2 is inside

    expect(engineMock.seek).toHaveBeenCalledWith(4.9);
    expect(result.current.state.loopRange).toEqual({
      firstSegmentId: SEG1.id,
      lastSegmentId: SEG2.id,
    });
  });

  it('outside band — clears the loop and seeks', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
    ...
    act(() => { result.current.controls.tapSegment(SEG3.id); }); // SEG3 is outside

    expect(result.current.state.loopRange).toBeNull();
    expect(engineMock.seek).toHaveBeenCalledWith(11.9); // segmentStartSec(SEG3)
  });
});

describe('grow/shrink controls', () => {
  it('growLoopUp extends the first segment backward', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
    ...
    act(() => { result.current.controls.growLoopUp(); });

    expect(result.current.state.loopRange).toEqual({
      firstSegmentId: SEG1.id,
      lastSegmentId: SEG3.id,
    });
  });

  it('growLoopUp is a no-op at the top of the list', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
  ...
  it('growLoopDown extends the last segment forward', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
    ...
    act(() => { result.current.controls.growLoopDown(); });
  ...
  it('growLoopDown is a no-op at the bottom of the list', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
  ...
  it('shrinkLoopUp drops the first segment', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
    ...
    act(() => { result.current.controls.shrinkLoopUp(); });
  ...
  it('shrinkLoopUp is a no-op at length 1', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));

    act(() => {
      result.current.dispatch({
        type: 'SET_LOOP',
        range: { firstSegmentId: SEG2.id, lastSegmentId: SEG2.id },
      });
    });
    act(() => { result.current.controls.shrinkLoopUp(); });
  ...
  it('shrinkLoopDown drops the last segment', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
    ...
    act(() => { result.current.controls.shrinkLoopDown(); });
  ...
  it('shrinkLoopDown is a no-op at length 1', () => {
    const { result } = renderHook(() => usePlayer(SEGS, 20000, '/audio'));
